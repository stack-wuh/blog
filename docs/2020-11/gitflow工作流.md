...
我深信，独立开发模式下的开发者们是不需要过多操心我们的代码管理的，更不会担心会不会我这一次的`合并会不会有冲突？` `有冲突了我怎么去改？` `代码改乱了我怎么办？`之类的问题。
...
#### Github Flow
GithubdFlow工作流的管理方式相当特殊，它只有一个分支master，而且受到了保护，有一个专门维护的开发者负责核查合并每一个pr。每一次merge操作之后，设置的github actions自动触发进行CI操作，通过后即可发布relaese版本。
...
执行--hard，然后commit之后会将当前指针之后的全部commit舍弃，而--soft不会，在修改commit之后，它任然会在保留提交的历史记录。
...
git rebase -i HEAD~~~~
```
我pick了最近的一次提交，将其他的commit 全部改为f后，现在提交已经全部合并进入了pick的top_commit_id了。这样我们可以将我们的功能分支全部管理起来，在合并进入dev分支之前将我们的代码处理的干干净净。

其操作结果如下图: 
![](https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-060713.png)

将代码处理干净了，没有多余提交了，我们可以来美化一下我们的master以及dev分支上面的历史提交记录啦。

在开始之前我们查看一下官网的描述, 关键的流程图已经做出了标记，如下图:
![](https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-061532.png)
...
```
其操作结果如下图: 
![](https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-070738.png)


我们分别看一下变基的`feat-module-name`分支与未变基的`feat-module-form`分支

**feat-module-name**
![](https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-073827.png)

**feat-module-form**
![](https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-073753.png)
...
Revert还可以执行一个骚操作，我们Revert了一次，达到了移除指定commit_id的操作，现在推迟发布上线，已经有时间去改了，我又需要将commit_id 的操作加上去。我们可以执行对一个Revert执行一次Revert，将指定的`反转`再一次`反转`，这样我们的代码不就还原啦。操作如下：
````bash
```bash
git log --oneline -10
...
注意:  
**1. 以master分支为主分支，收到分支保护，不允许任何人提交，由专人维护**  
**2. 全部分支以dev分支为上游源**

相关文章:  
[Git协同与提交规范][1](蚂蚁金服前端九部)    
[Rebase(变基)](https://www.liaoxuefeng.com/wiki/896043488029600/1216289527823648)(跟着廖雪峰学习Git)    
[Rebase变基](https://git-scm.com/book/zh/v2/Git-%E5%88%86%E6%94%AF-%E5%8F%98%E5%9F%BA)(Git官网)   
...
[image-8]:	https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-035633.png
[image-9]:	https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-040604.png
[image-10]:	https://source-1300700534.cos.ap-shenzhen-fsi.myqcloud.com/2020-11-15-043651.png